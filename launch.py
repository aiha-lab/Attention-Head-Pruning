...
def parse_args():
    ...
    parser.add_argument(
        "--nproc_per_node",
        type=int, default=1, help="The number of processes to launch on each node, "
    ...
    parser.add_argument(
        "--stdout_all", action='store_true', help="Print all outputs in console"
    )
...
def main():
    ...
    if '--gpus' in args.training_script_args:
        ...
        args.training_script_args += ['--gpus', str(args.nproc_per_node)]
