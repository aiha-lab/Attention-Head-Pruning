...
class LMTrainer(BaseTrainer):
    ...
    def __init__(self, *args, length_config: dict,
                 l0_coefficient: float, l0_start_iters: int, l0_warmup_iters: int,
                 **kwargs):
        ...
        self.l0_start_iters = l0_start_iters
        self.l0_warmup_iters = l0_warmup_iters
        print_log(f'L0 regularization: {l0_coefficient} (start: {l0_start_iters}, warmup: {l0_warmup_iters})')
    ...
    def train_epoch_body(self):
        ...
        with torch.enable_grad():
            ...
            for train_iter, (train_input, train_label, train_seq_len) in enumerate(self.train_dataloader):
                ...
                if self.current_iteration < self.l0_start_iters:
                    l0_coeff = 0.0
                elif self.current_iteration < self.l0_start_iters + self.l0_warmup_iters:
                    l0_coeff = self.l0_coefficient * (
                            self.current_iteration - self.l0_start_iters + 1) / self.l0_warmup_iters
                else:
                    l0_coeff = self.l0_coefficient
                ...
                if is_master():
                    if (train_iter % self.print_interval == 0) and (train_iter > 0):
                        ...
                        s += f'......... loss(batch/avg): ' \
                             ...
                             f'......... l0 loss(batch/avg), (coefficient: {l0_coeff:.6f}/{self.l0_coefficient:.6f}): ' \
                             f'{l0_loss.item():.6f} / ' \
    ...
    def valid_body(self, *, track: bool = True):
        ...
        if is_master():
            for p_name, p_val in self.model.named_parameters():
                if 'gate' in p_name:
                    negative_p_val = torch.less_equal(p_val, 0.0).int().sum()
                    print_log(f'GATE parameters {p_name}: {p_val}')
                    print_log(f'GATE sparsity {negative_p_val.item()} / {p_val.numel()}')
...
def run(cfg: Dict):
    ...
    network.set_gating(True)  # KEY PART
    ...
    for param_name, param_val in network.named_parameters():
        if 'norm' in param_name:
            norm_params.append(param_val)
        elif 'gate' in param_name:
            gate_params.append(param_val)
        else:
    ...
    params_for_opt = [
        ...
        {'params': gate_params, 'lr': cfg['optimizer']['lr'] * 10, 'weight_decay': 0.0},
    ]
    ...
    trainer = LMTrainer(
        ...
        l0_start_iters=int(cfg['custom']['l0_start_epoch'] * len(train_dataloader)),
        l0_warmup_iters=int(cfg['custom']['l0_warmup_epoch'] * len(train_dataloader)),
    ...
    if cfg['resume']:
        ...
        for n in range(network.num_layers):
            n_key = f'layers.{n}.attn.gate.gate.data'
            if n_key not in new_ckpt['network']:
                # dummy_gate = torch.zeros(network.num_heads, dtype=torch.float32, device=device)
                dummy_gate = torch.ones(network.num_heads, dtype=torch.float32, device=device).fill_(2.0)
                new_ckpt['network'][n_key] = dummy_gate
